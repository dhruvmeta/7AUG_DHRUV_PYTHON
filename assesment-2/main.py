...
class database:
    def __init__(self):
        self.conn = pymysql.connect(host="localhost", user="root", password="", database="tkinter_database", cursorclass=pymysql.cursors.DictCursor)
...
class Bill_App:
    ...
    def bill_area(self):
        ...
        self.database.execute_query("INSERT INTO bill(`Costumer name`,`Phone Number`,`Bill No.`,`Bath Soap`,`Face Cream`,`Face Wash`,`Hair Spray`,`Body Lotion`,`Rice`,`Food Oil`,`Daal`,`Wheat`,`Sugar`,`Maza`,`Coke`,`Frooti`,`Nimkoos`,`Biscuits`,`Total Cosmetics(in Rs.)`,`Total Grocery(in Rs.)`,`Other Total(in Rs.)`,`Cosmetics Tax(in Rs.)`,`Grocery Tax(in Rs.)`,`Other Tax(in Rs.)`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
            self.cus_name.get(),
            self.c_phone.get(),
            self.c_bill_no.get(),
            self.bath_soap.get(),
            ...
            self.total_grocery_prices,
            self.total_other_prices,
            self.total_cosmetics_prices * 0.05,
